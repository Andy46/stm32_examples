...
//#define BEGIN_COM digitalWrite(_ss, LOW); HAL_Delay_us(1)
//#define END_COM   HAL_Delay_us(1); digitalWrite(_ss, HIGH)
...
const unsigned char firmware_data[] = {    // SROM 0x04
...
//  SPI.begin();
  ...
  HAL_Delay_us(1);
  HAL_GPIO_WritePin(PMW_CS_GPIO_Port, PMW_CS_Pin, GPIO_PIN_SET); // ensure that the serial port is reset
  HAL_Delay_us(1);
...
//  SPI_BEGIN;
  ...
  HAL_SPI_Receive(&hspi1, burstBuffer, sizeof(burstBuffer), 1000);
...
*/
uint8_t PMW3360::readReg(uint8_t reg_addr)
{
//  SPI_BEGIN;
...
*/
void PMW3360::writeReg(uint8_t reg_addr, uint8_t data)
{
//  SPI_BEGIN;
...
*/
uint8_t PMW3360::adns_read_reg(uint8_t reg_addr)
{
  ...
  uint8_t buffer[2];
  buffer[0] = (reg_addr & (uint8_t)0x7f);
  buffer[1] = 0;


  HAL_GPIO_WritePin(PMW_CS_GPIO_Port, PMW_CS_Pin, GPIO_PIN_RESET);

  HAL_SPI_TransmitReceive(&hspi1, buffer, buffer, sizeof(buffer), 1000);
  HAL_Delay_us(100); // tSRAD is 25, but 100us seems to be stable.

  HAL_GPIO_WritePin(PMW_CS_GPIO_Port, PMW_CS_Pin, GPIO_PIN_SET);
  HAL_Delay_us(19); //  tSRW/tSRR (=20us) minus tSCLK-NCS
...
*/
void PMW3360::adns_write_reg(uint8_t reg_addr, uint8_t data) {
  if(reg_addr != REG_Motion_Burst)
  {
    _inBurst = false;
  }
  ...
  // Send address of the register, with MSBit = 1 to indicate it's a write
  uint8_t buffer[2];
  buffer[0] = (reg_addr | (uint8_t)0x80);
  buffer[1] = data;
  ...
  HAL_Delay_us(20); // tSCLK-NCS for write operation
  HAL_GPIO_WritePin(PMW_CS_GPIO_Port, PMW_CS_Pin, GPIO_PIN_SET);
//  HAL_Delay_us(100); // tSWW/tSWR (=120us) minus tSCLK-NCS. Could be shortened, but is looks like a safe lower bound
}
...
void PMW3360::adns_upload_firmware() {
  ...
  for (int i = 0; i < SROM_LENGTH; i++) {
    c = srom[i]; //(unsigned char)pgm_read_byte(firmware_data + i);
    HAL_SPI_Transmit(&hspi1, &c, sizeof(c), 1000);
    HAL_Delay_us(16);
  }
...
*/
//uint8_t PMW3360::readImagePixel()
//{
//  uint8_t pixel = SPI.transfer(0);
//  HAL_Delay_us(20);
//
//  return pixel;
//}
